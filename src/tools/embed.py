...
"""
Embed Texts Agent Tool for DeepSearchAgents.
"""
...
import torch
from src.core.ranking.jina_embedder import JinaAIEmbedder
...
class EmbedTextsTool(Tool):
    ...
    inputs = {
        "texts": {
            "type": "string",
            "description": (
                "A JSON string representing a list of texts to embed."
            ),
        },
        "model": {
            "type": "string",
    ...
    def forward(
        ...
        """
        Execute text embedding and return a JSON string of the list of
        embedding vectors.
        ...
        log_func(f"[bold blue]Executing text embedding (model: "
                 f"{effective_model})[/bold blue]")
        ...
        try:
            ...
            try:
                ...
                if not input_list:
                    log_func("[yellow]Input text list is empty, "
                             "returning empty list.[/yellow]")
                    return "[]"
                log_func(f"[dim]Number of texts to embed: "
            ...
            async def run_embed():
                async with embedder:
                    ...
                    tensor_result: torch.Tensor = (
                        await embedder.get_embeddings_async(
                            input_list,
                            task=task,
            ...
            embeddings_list: List[List[float]] = loop.run_until_complete(
                run_embed()
            )
            ...
            log_func(f"[bold red]Error during text embedding: "
                     f"{str(e)}[/bold red]")
